...
class VehicleTypeScreen extends StatefulWidget {
  ...
  VehicleTypeScreen({required this.token, required this.vehicle_type});
...
class _VehicleTypeScreenState extends State<VehicleTypeScreen> {
  ...
  void initState() {
    super.initState();
    _description = widget.vehicle_type.description;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.vehicle_type.id == 0 ? 'Nuevo Tipo de Documento' : widget.vehicle_type.description
        ),        
      ),
      body: Stack(
        children: [
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          widget.vehicle_type.id == 0 ? Container() : SizedBox(width: 20),
          widget.vehicle_type.id == 0 
          ? Container() 
          : Expanded(
              child: ElevatedButton(
                child: Text('Borrar'),
  ...
  void _save() {
    ...
    widget.vehicle_type.id == 0 ? _addRecord() : _saveRecord();
  }
  ...
  _saveRecord() async {
    ...
    Map<String, dynamic> request = {
      'id': widget.vehicle_type.id,
      'description': _description,
    };

    Response response = await ApiHelper.put(
      '/api/VehicleTypes/',
      widget.vehicle_type.id.toString(),
      request,
      widget.token.token 
    );
